    ...
    for (int i = 0; i < widget.length; i++) {
      _focusNodes[i].addListener(() {
        if (_focusNodes[i].hasFocus) {
          ...
          // If trying to focus a slot after an empty one, redirect focus
          if (firstEmptyIndex != -1 && i > firstEmptyIndex) {
            _focusNodes[firstEmptyIndex].requestFocus();
            return; // Skip setState to avoid flicker, let the new focus handle it
          }
        }
        if (mounted) setState(() {});
      });
          ...
          return Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isFocused ? Colors.white : AppColors.greyBackground,
